...
import math
import random
from abc import ABCMeta, abstractmethod   #abc用于定义抽象基类
...
def g_ext_norm(vec, m):      #L2-ALSH P变化扩展项   对应1.8
    l2norm_square = dot(vec, vec)
    return [l2norm_square**(i+1) for i in xrange(m)]

def g_ext_half(m):			#L2-ALSH Q变化扩展项	对应1.8
    return [0.5 for i in xrange(m)]

def g_ext_norm_cosine(vec, m):    #sign-ALSH P变化扩展项  对应2.15
    l2norm_square = dot(vec, vec)
    return [0.5 - l2norm_square**(i+1) for i in xrange(m)]

def g_ext_norm_simple(vec, m):	#simple-ALSH P变化扩展项  对应3.9
    l2norm_square = dot(vec, vec)
    return [np.sqrt(1 - l2norm_square) for i in xrange(m)]

def g_ext_zero(m):   #sign-ALSH Q变化扩展项  对应2.16
...
# normalization for each query   #将query 查询归一化
...
class L2Lsh(Hash):
    ...
    def hash(self, vec):	# hash family 算法
