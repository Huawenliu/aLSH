...
std::string hash_combine(const std::vector<double> &hashes) {
...
MatrixXd g_normalization(const MatrixXd &queries) {
    const double U = 0.83
    //U = 0.75
    MatrixXd norm_queries(queries.rows());
    double ratio;
    for(int i=0;i<queries.rows();i++) {
...
double L2Lsh::hash(const VectorXd &vec) {
	return (dot(vec, data_) + b_) / r_;
}
...
double ConsineLsh::hash(const VectorXd &vec) {
	double dot_val = dot(vecd, data_);
	if (dot_val > 0.0) {
		return 1.0;
	}
	return 0.0;
...
double ConsineLsh::distance(const VectorXd &u, const VectorXd &v) {
	return 1 - sqrt(dot(u, v) / (dot(u, u) * dot(v, v)));
}
